...
def userInput():
    string = input("Please enter a 100 character long string")
    print()
    return string
...
def checkBackward(userString, findWordList):
    reverseUserString = flipString(
        findWordList)  # Flips the word list so it can find the word in reverse form in the 100 character string
    puzzle = []
    string_result = ""
    ...
    for string in range(len(puzzle)):  # string is my row
        if (puzzle[string].find(reverseUserString)) > -1:
            index = puzzle[string].find(
                reverseUserString)  # index is the column but since it is reverse we have to add the length of the
            # string-1 to get actual position
            string_result = "{0}: {1} row: {2} column: {3}".format(
...
def checkUp(string, findWordList):
    trans_string = transpose_string(string, 10)  # Transposes the string and flips the string to make the string
    # display the wanted word in the right away
    ...
    for string in range(len(puzzle)):
        if puzzle[string].find(
                findWordList) > -1:  # row is still the index; but since it is reversed for row you have to minus by
            # the final index(9)
